...
var fromImpliedDecimal = require("./numberUtils").fromImpliedDecimal;
...
Types.version = {
    fromByteBuffer: function fromByteBuffer(b) {
        var m = b.readUint8(0);
        var h = b.readUint8(1);
        var r = b.readUint16(2); 
        return m + '.' + h + '.' + r;        
    }, 
    appendByteBuffer: function appendByteBuffer(b, object) {
        object = object.trim()
        if (! /^[0-9]+\.[0-9]+\.[0-9]+$/.test(object)) {
            throw new Error("Version must be in format 0.0.0")
        }
        var m = object.split(".")[0];
        var h = object.split(".")[1];
        var r = object.split(".")[2];        

        var value = (m << 24) | (h << 16) | r;
        b.writeUint32(value);
        return;
    },
    ...
    toObject: function toObject(object) {
        ...
        if (debug.use_default && object === undefined) { return "0.0.0"; }
        return object
    }
}
...
Types.vote_id = { TYPE: 0x000000FF,
    ...
    fromObject: function fromObject(object) {
        ...
        var type = _object$split[0];
        var id = _object$split[1];
...
Types.static_variant = function (_st_operations) {
    return {
        ...
        getId: function getId (t) {
            ...
            for (var i=0, l=this.st_operations.length; i<l; i++) {
                if (this.st_operations[i].operation_name === t) return i;
            }
        },
        fromByteBuffer: function fromByteBuffer(b) {
        ...
        appendByteBuffer: function appendByteBuffer(b, object) {
            v.required(object);
            var type_id = this.getId(object[0]);
            var st_operation = this.st_operations[type_id];
            v.required(st_operation, 'operation ' + type_id);
        ...
        compare: function compare (a, b) {
            return strCmp(this.getId(a[0]), this.getId(b[0]));
        }
    };
};
...
Types.public_key = {
    toPublic: function toPublic(object) {
        ...
        return object == null ? object : object instanceof PublicKey ? object : PublicKey.fromStringOrThrow(object);
    },
    fromByteBuffer: function fromByteBuffer(b) {
