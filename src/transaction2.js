...
var Transaction = function (opts) {
    ...
    this.ref_block_num = opts.ref_block_num || 0;
    this.ref_block_prefix = opts.ref_block_prefix || 0;
    this.expiration = opts.expiration || 0;
    this.operations = [];
    if (opts.operations) this.addOperations(opts.operations);
...
Transaction.Id = function (opts) {
	return (new Transaction(opts)).Id();
}
...
Transaction.prototype.signWithAccount = function (accountName, role) {
	...
	if (!this.tr_buffer) throw new Error("not complete");
	...
	//console.log('signing with:', prikey.toPublicKey().toString())
...
Transaction.prototype.broadcast = function (callback) {
	(!this.tr_buffer) return this.processBroadcast(callback);
	this.remote.transactionManager.broadcast(this, callback);
}

Transaction.prototype.processBroadcast = 
...
Transaction.prototype.submit = function (callback) {
	(!this.tr_buffer) return this.processSubmit(callback);
...
Transaction.prototype.addOperation = function (name, params) {
    ...
    if (name === 'proposal_create') {
        params.expiration_time || (params.expiration_time = Math.round(Date.now()/1000 + Config.expire_in_secs_proposal));
    }
